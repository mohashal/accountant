...
include_once 'clasat/instant_pay.php';
...
<script src="js/instant_pay.js"></script>';
$ready="
...
case 'save_temp_pay':$m=$ins->instant_save_temp($mysqli);if($m[1]==0){$hf->msg($m[0],$m[1],'fail');}else{$hf->msg($m[0],$m[1],'success','instant_pay.php?conn=add_bill_form');}break;
case 'temp_bill':$ins->show_temp_bill($mysqli);break;
case 'edit_bill':$ins->edit_instant_form($mysqli, intval($_GET['id']));break;
case 'save_edit_pay':$m=$ins->instant_save_edit($mysqli);if($m[1]==0){$hf->msg($m[0],$m[1],'fail');}else{$hf->msg($m[0],$m[1],'success','instant_pay.php?conn=temp_bill');}break;
