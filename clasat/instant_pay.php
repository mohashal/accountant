...
*/
function add_bill_form($mysqli,$action=null,$type=null){
$re=$this->get_partner_employee($mysqli,99999,99999);
echo "
<div class='form_main'>
    <form action='instant_pay.php?conn=save_temp_pay' method='post' enctype='multipart/form-data'>
    <div class='form_main_name'>".langu['instant_pay']."</div>
...
$q=$mysqli->query('select instant_invoice.*,merchants.name as mname,merchants.balance from instant_invoice,merchants where merchants.id=instant_invoice.merchant_id and instant_invoice.id='.$id);
$row=$q->fetch_assoc();
...
echo "
<div class='form_main'>
    <form action='instant_pay.php?conn=save_edit_pay' method='post' enctype='multipart/form-data'>
    <div class='form_main_name'>".langu['instant_pay']."</div>
    <div class='form_main_inputs'>
...
if($s=$mysqli->prepare("INSERT INTO instant_invoice (profit,products,notes,customer_name,merchant_id,total,date,partner_id,employee_id) VALUES (?,?,?,?,?,?,?,?,?)")){
...
if($s=$mysqli->prepare("update instant_invoice set profit=?,products=?,notes=?,customer_name=?,merchant_id=?,total=?,date=?,partner_id=?,employee_id=? where id=". intval($_POST['instance_id']))){
@$s->bind_param("dsssidsii",$profit_all,$a,htmlentities($_POST['notes']),htmlentities($_POST['customer_name']),$merchant,$total,$date,$partner,$employee);
$s->execute();
...
$q=$mysqli->query('SELECT * FROM instant_invoice where instant_invoice.is_temp=1');
if($q->num_rows>0){
$profit=0;
echo "
...
echo "
    <tr class='show_hide'>
        ...
        <td class='products_list_symbol no_print' onClick='login_modal(\"".langu['save_temp']."\",\"ajaxbills.php?update_temp_bills=instant&id=".$row['id']."\")'>✔</td>
        <td>".$name."</td>
        <td class='nume'>".$row['total']."</td>
        <td class='nume'>".$row['profit']."</td>
        <td class='products_list_symbol no_print'><a href='instant_pay.php?conn=edit_bill&id=".$row['id']."'></a></td>
        <td class='products_list_symbol no_print'><a href='printbills.php?print_bills=instant&id=".$row['id']."' target='_blank'>|</a></td>
...
echo"
        ...
        <div class='form_input_line' style='margin:15px auto;'><div class='hide_show nprint' style='' onclick='save_select_temp(\"saveit\",\"ajaxbills.php?update_temp_bills=instant&id=\")'>".langu['save_temp']."</div><div class='hide_show nprint' style='' onclick='window.print()'>".langu['print']."</div></div>
</div>
";
}
